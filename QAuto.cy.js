...
describe ('QAuto',()=>{
    it('Create User',()=>{
        ...
        cy.get('body > app-root > app-global-layout > div > div > app-header > header > div > div > div.header_right.d-flex.align-items-center > button.btn.btn-outline-white.header_signin').should('have.text','Sign In');
        ...
        cy.get('#signupName').type('White').should('have.value', 'White');
        cy.get('#signupLastName').type('Black');
        cy.get('#signupEmail').type('WBlack211@gmail.com').invoke('val').should('include', '@');
        cy.get('#signupPassword').type('WBl1234567');
        cy.get('#signupRepeatPassword').type('WBl1234567').should('have.value', 'WBl1234567');
        ...
        cy.get('#addCarBrand').should('have.prop', 'tagName', 'SELECT');
        cy.get('#addCarBrand').select('Ford');
        cy.get('#addCarModel').should('have.prop', 'tagName', 'SELECT');
        cy.get('#addCarModel').select('Fiesta');
        cy.get('#addCarMileage').should('have.attr', 'type', 'number');
        ...
        cy.get('body > ngb-modal-window > div > div > app-remove-account-modal > div.modal-footer.d-flex.justify-content-end > button.btn.btn-danger').should('have.text','Remove');
